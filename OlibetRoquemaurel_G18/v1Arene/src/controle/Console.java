...
/**
 * 
 * Se connecte au serveur avec un Element et sa VueElement. "run" permet a
 * l'Element/VueElement de se deplacer
 * 
 */
public class Console extends UnicastRemoteObject implements IConsole {
	...
	private static final int port = 5099; // port par defaut pour communiquer
											// avec le serveur RMI
	private Remote serveur = null; // le serveur avec lequel le controleur
									// communique
	private VueElement ve = null; // la vue de l'element (pour l'interface
									// graphique)
	private Element elem = null; // l'element pour lequel le controleur est cree
	private Hashtable<Integer, VueElement> voisins; // les vues des voisins sur
													// l'interface graphique
	private Point pointErrance; // le point ou aller errer
	private int refRMI; // la reference (entiere) attribuee par le serveur a la
						// connexion
	...
	public Console(Element elem, int dx, int dy) throws RemoteException {
		...
		try {
			...
			// Creation de la position initiale de la vue de l'element sur
			// l'interface graphique
			...
			System.out
					.println("Erreur : la console n'a pa pu etre creee !\nCause : "
							+ e.getCause());
			e.printStackTrace();
		}
	}
	...
	/**
	 * Permet au serveur de faire "jouer" un tour a l'element. Calcule ses
	 * voisins (donnes par le serveur), cherche le plus proche, s'il est a
	...
	public void run() throws RemoteException {
		...
		//if (voisin = objet and nbobjmax atteint)
		// Hashtable<Integer,VueElement> voisinsInconnus =
		// extraireInconnus(voisins);

		// Recherche du plus proche, sinon errer

		HashMap<Integer, HashMap<Integer, VueElement>> resultat = Strategie
				.chercherElementProche(ve, voisins);
		int distPlusProche = resultat.keySet().iterator().next();
		int refPlusProche = resultat.get(distPlusProche).keySet().iterator()
				.next();
	...
	public void seDirigerVers(int ref) {
		...
		// sinon la cible devient le point sur lequel se trouve l'element le
		// plus proche
		else {
			pvers = voisins.get(ref).getPoint();
		}

		// si l'element n'existe plus (cas posible: deconnexion du serveur), le
		// point reste sur place
		if (pvers == null)
			return;

		// calcule la direction pour atteindre la ref (+1/-1 par rapport a la
		// position courante)
		...
		if (UtilitaireConsole.caseVide(dest, voisins)) {
			...
			// cherche la case libre la plus proche dans la direction de la
			// cible
			Point top = UtilitaireConsole.meilleurPoint(ve.getPoint(), dest,
					voisins);
	...
	public void perdreVie(int viePerdue) throws RemoteException {
		...
		System.out.println("Ouch, j'ai perdu " + viePerdue
				+ " points de vie. Il me reste " + this.elem.getVie()
				+ " points de vie.");
	}
	...
	/**
	 * Ajout l'element dans la liste des elements connus (combattants et
	 * equipements)
	 * 
	 * @param ref
	 *            l'element a ajouter
